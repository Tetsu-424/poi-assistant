...
高校時代に一週間程度かけて作ったものを大学に入るまえの暇なときに少しいじりました。
今回はその際に実装したかった「入力内容の保持」を実装しようと思いました。（真ん中の要素で実装しました。）

-機能概要-
左側の要素は様々なお役立ちサイトを載せています。（個人利用目的で作ったので勝手に載せています）
...
各行ごとにリセットボタンを押すことでリセットできます。
最初は真ん中にゲーム画面を埋め込んで、このページで遊べるようにしていたのですが、ゲーム側のFlash→html5移行によってエラーを吐くようになってしまいました。
艦これのソースコードからゲームが動いている領域のみのURLを探しだすところまでは行きましたが（htmlのコードにコメントアウトしてます）、丸一日頑張ってもhtml5版の艦これを埋め込んで動かすことができず、今回は上記の新機能を実装するに至りました。
...
ローカル環境では通知が飛ばないので、サーバーを立てることで通知を確認できます。
...
4.使用者のモニターの大きさやウィンドウの大きさによってスタイルがぐちゃぐちゃになる（CSSの知識不足）。
5.Chrome以外のブラウザでのデバックはできていないため、未知のバグがあるかもしれない。
6.変数名や関数で似通っている名前をつけてしまったものが多いため、コードが煩雑でわかりにくい、繰り返しのコードや文が多い。
7.真ん中の要素のリセットボタンはGarlicの仕様上送信ボタンを偽装しているので、毎回ページが更新されてしまう。
8.DBが古い
等挙げればきりがないです。